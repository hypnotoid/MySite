...
public class ProductValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (product.getName().isBlank())
            errors.rejectValue("name", "", input + " имя");
        if (!product.getName().matches("[a-zA-Zа-яА-ЯёЁ0-9 \\-]+"))
            errors.rejectValue("name", "", "Имя" + invChar);
        if (product.getName().length() < 3)
            errors.rejectValue("name", "", lenMin + "3-х символов");
        if (product.getName().length() > 100)
            errors.rejectValue("name", "", lemMax + "100 символов");


        if (product.getAmount() > Integer.MAX_VALUE-1 )
            errors.rejectValue("amount", "", "Слишком много");
        if (product.getAmount() < Integer.MIN_VALUE + 2 )
            errors.rejectValue("amount", "", "Слишком мало");
        if (product.getPrice()> 100000000 )
            errors.rejectValue("price", "", "Слишком много");
        if (product.getPrice() < 0.01)
            errors.rejectValue("price", "", "Не менее 0,01");
        if (product.getId() < 0)
            errors.rejectValue("name", "", "Внутренняя ошибка");
    }
}
